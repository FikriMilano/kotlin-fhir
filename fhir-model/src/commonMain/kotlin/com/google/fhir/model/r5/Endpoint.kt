...
public data class Endpoint(
  ...
  public val environmentType: List<CodeableConcept> = listOf(),
  /**
  ...
  public data class Payload(
    ...
    public class Builder() {
       ...
       * The payload type describes the acceptable content that can be communicated on the endpoint.
       *
       * The mimeType describes the serialization format of the data, where the payload.type
      ...
      /**
       * The mime type to send the payload in - e.g. application/fhir+xml, application/fhir+json. If
       * the mime type is not specified, then the sender could send any content (including no
       * content depending on the connectionType).
       *
       * Sending the payload has obvious security consequences. The server is responsible for
  ...
  public class Builder(
     ...
     * endpoint as not currently valid. Temporary downtimes or other unexpected short-term changes
     * in availability would not be represented in this property.
     */
     ...
     * as in the XDS example. If there are multiple payload types or mimetypes they are all
     * applicable for all connection types, and all have the same status.
     */
    public var connectionType: MutableList<CodeableConcept.Builder>,
    /**
     * The uri that describes the actual end-point to connect to.
     *
     * For rest-hook, and websocket, the end-point must be an http: or https: URL; for email, a
    ...
    /**
     * The description of the endpoint and what it is for (typically used as supplemental
     * information in an endpoint directory describing its usage/purpose).
     */
    public var description: String.Builder? = null

    /** The type of environment(s) exposed at this endpoint (dev, prod, test, etc.). */
    public var environmentType: MutableList<CodeableConcept.Builder> = mutableListOf()
     ...
     * The set of payloads that are provided/available at this endpoint.
     *
     * Note that not all mimetypes or types will be listed under the one endpoint resource, there
     * may be multiple instances that information for cases where other header data such as the
     * endpoint address, active status/period etc. is different.
     */
